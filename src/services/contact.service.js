...
function getMiniContacts() {
    return gDefaultContacts.map(({ name, _id }) => ({ name, _id }))
}

function getPrevNextId(id) {
    return new Promise((resolve, reject) => {
        const contactIdx = gContacts.findIndex(contact => contact._id === id)
        let prevId, nextId
        if (contactIdx === 0) prevId = gContacts[gContacts.length - 1]._id
        else if (contactIdx === gContacts.length - 1) nextId = gContacts[0]._id
        contactIdx !== -1 ? resolve({ prevId: gContacts[contactIdx - 1]?._id || prevId, nextId: gContacts[contactIdx + 1]?._id || nextId })
...
function _loadContacts() {
    ...
    if (!contacts || !contacts.length) {
        ...
        contacts.forEach(contact => {
            contact.imgUrl = `https://robohash.org/set_set5/${utilService.getRandomIntInclusive(1, 500)}.png`
        })
    }
